...
def clean_results_folder(vbl):
    folder = f"/exec_env/examples/{vbl}/fuzzer/reporter"
    if os.path.isdir(folder):
        delete_folder_content(f"/exec_env/examples/{vbl}/fuzzer/reporter")
        os.mkdir(f"/exec_env/examples/{vbl}/fuzzer/reporter/bug")


def get_explored_vulnerabilities(vbl, results_file):
    contracts = []
    f = open(
        f"/exec_env/examples/{vbl}/fuzzer/reporter/bug/{results_file}.list", "r")
    lines = f.readlines()
    for line in lines:
...
def get_tested_contracts(vbl):
    contracts = []
    f = open(f"/exec_env/examples/{vbl}/fuzzer/config/contracts.list", "r")
    lines = f.readlines()
    for line in lines:
        contracts.append(line.strip())
    temp = set(contracts)
...
def get_contract_size(vulnerability, contract_name):
    size = -1
    if os.path.isfile(f"/exec_env/examples/{vulnerability}/verified_contract_bins/{contract_name}.bin"):
        size = os.path.getsize(
            f"/exec_env/examples/{vulnerability}/verified_contract_bins/{contract_name}.bin") / 2
    if os.path.isfile(f'/exec_env/examples/{vulnerability}/verified_contract_bins/{contract_name}.bin-runtime'):
        size = os.path.getsize(
            f"/exec_env/examples/{vulnerability}/verified_contract_bins/{contract_name}.bin-runtime") / 2
...
def get_num_functions(vulnerability, contract_name):
    num = -1
    if os.path.isfile(f"/exec_env/examples/{vulnerability}/verified_contract_abis/{contract_name}.abi"):
        f = open(
            f"/exec_env/examples/{vulnerability}/verified_contract_abis/{contract_name}.abi", "r")
        abi = json.load(f)
...
def main(reps=1):
    ...
    for _ in range(reps):
        ...
        for key in bugs:
            ...
            start = time.time()
            os.environ["CONTRACT_DIR"] = f'/exec_env/examples/{key}'
            result = subprocess.call([f'/exec_env//fuzzer_run.sh'])
            ...
            perc = compute_percentage(
                explored_contracts, tested_contracts.copy())
    ...
    f = open('/exec_env/output/results.json', 'w')
