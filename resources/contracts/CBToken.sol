...
contract CBToken is StandardToken {
    ...
    uint256 public totalSupply = 1000000000 * (10**uint256(decimals)); //Total supply
    uint256 public sellPrice = 1000000000000000 wei; //Tokens are sold for this price
    ...
    mapping(address => uint256) public specialBonus;
    ...
    uint256 public currentBonus = 0; //Current bonus to tokens purchases

    //Technical variables to store statistical data
    uint256 public StatsEthereumRaised = 0 wei; //Total Ethereum raised
    uint256 public StatsSold = 0; //Sold tokens amount
    uint256 public StatsMinted = 0; //Minted tokens amount
    uint256 public StatsTotal = 0; //Overall tokens amount
    ...
    event Mint(address indexed from, uint256 tokens); // This notifies clients about the amount minted
    event Burn(address indexed from, uint256 tokens); // This notifies clients about the amount burnt
    event PriceChanged(string _text, uint256 _tokenPrice); //Manual token price
    event BonusChanged(string _text, uint256 _percent); //Sale bonus percent for each purchase
    ...
    function changeContractUpgradability(bool _contractUpgradable)
        external
        onlyOwner
    {
    ...
    function functionOne(
        ...
        uint256 _valueThree,
        uint256 _valueFour
    ) onlyPayloadSize(4) returns (bool ok) {
        require(contractUpgradable); //Upgradability should be enabled
        return
            delegateContract.delegatecall(
                bytes4(sha3("functionOne(uint256,uint256,uint256,uint256)")),
                _valueOne,
                _valueTwo,
                _valueThree,
                _valueFour
    ...
    function functionTwo(
        ...
        return
            delegateContract.delegatecall(
                bytes4(sha3("functionTwo(uint256,uint256,uint256,uint256)")),
                _valueOne,
                _valueTwo,
                _valueThree,
                _valueFour
    ...
    function functionThree(
        ...
        return
            delegateContract.delegatecall(
                bytes4(sha3("functionThree(uint256,uint256,uint256,uint256)")),
                _valueOne,
                _valueTwo,
                _valueThree,
                _valueFour
    ...
    function functionFour(
        ...
        return
            delegateContract.delegatecall(
                bytes4(sha3("functionFour(uint256,uint256,uint256,uint256)")),
                _valueOne,
                _valueTwo,
                _valueThree,
                _valueFour
    ...
    function functionFive(
        ...
        return
            delegateContract.delegatecall(
                bytes4(sha3("functionFive(uint256,uint256,uint256,uint256)")),
                _valueOne,
                _valueTwo,
                _valueThree,
                _valueFour
    ...
    function functionSix(
        ...
        return
            delegateContract.delegatecall(
                bytes4(sha3("functionSix(uint256,uint256,uint256,uint256)")),
                _valueOne,
                _valueTwo,
                _valueThree,
                _valueFour
    ...
    function setBonus(uint256 _percent) external {
        require(msg.sender == owner || msg.sender == minter);
        require(_percent >= 0);
    ...
    function setSpecialBonus(address _target, uint256 _percent) external {
        require(msg.sender == owner || msg.sender == minter);
        require(_percent >= 0);
    ...
    function mintTokens(address _target, uint256 _amount)
        external
        returns (bool)
    {
        require(msg.sender == owner || msg.sender == minter);
    ...
    function decreaseTokens(address _target, uint256 _amount)
        external
        returns (bool)
    {
        require(msg.sender == owner || msg.sender == minter);
    ...
    function buy() public payable returns (bool) {
        ...
        uint256 tokens = msg.value / price(); //Number of tokens to be received by the buyer
        require(tokens > 0); //Number of tokens must be greater than 0

        //Add bonus tokens
        if (currentBonus > 0) {
            uint256 bonus = safeMul(tokens, currentBonus);
        ...
        if (specialBonus[msg.sender] > 0) {
            uint256 addressBonus = safeMul(tokens, specialBonus[msg.sender]);
            addressBonus = safeDiv(addressBonus, 100);
            tokens = safeAdd(addressBonus, tokens);
        }

        uint256 tokensToAdd = tokens * (10**uint256(decimals));
