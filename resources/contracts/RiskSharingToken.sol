...
contract FeesControllerBase is RSTBase {
    ...
    function setRepayment() public payable;
}
...
contract RiskSharingToken is RSTBase {
    ...
    function RiskSharingToken(address _board) {
        ...
        weiForToken = uint256(10)**(18 - 1 - decimals); // 0.1 Ether
        reserve = 0;
        crr = 20;
        totalAccounts = 0;
    }
    ...
    function setTokenController(TokenControllerBase tc, address _tokenData)
        public
        boardOnly
    {
        tokenController = tc;
        if (_tokenData != address(0)) tokenData = _tokenData;
        if (tokenController != TokenControllerBase(0))
            if (!tokenController.delegatecall(bytes4(sha3("init()")))) revert();
    }
    ...
    function startVoting(
        bytes32 /*description*/
    ) public boardOnly validAddress(votingController) {
        if (!votingController.delegatecall(msg.data)) revert();
    }
    ...
    function buy() public payable validAddress(tokenController) {
        if (!tokenController.delegatecall(msg.data)) revert();
    }

    function sell(
        uint256 /*value*/
    ) public validAddress(tokenController) {
        if (!tokenController.delegatecall(msg.data)) revert();
    }
    ...
    function setFeesController(FeesControllerBase fc) public boardOnly {
        feesController = fc;
        if (!feesController.delegatecall(bytes4(sha3("init()")))) revert();
    }

    function withdrawFee() public validAddress(feesController) {
        if (!feesController.delegatecall(msg.data)) revert();
    }
    ...
    function addPayee(
        address /*payee*/
    ) public validAddress(feesController) {
        if (!feesController.delegatecall(msg.data)) revert();
    }
    ...
    function setRepayment() public payable validAddress(feesController) {
        if (!feesController.delegatecall(msg.data)) revert();
    }
}
