...
contract Object is Owned {
    ...
    uint256 constant OWNED_ACCESS_DENIED_ONLY_CONTRACT_OWNER = 8;

    function withdrawnTokens(address[] tokens, address _to)
        onlyContractOwner
        returns (uint256)
    {
    ...
    function checkOnlyContractOwner() internal constant returns (uint256) {
        if (contractOwner == msg.sender) {
            return OK;
        }
